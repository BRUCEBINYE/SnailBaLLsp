...
def preprocess_labels(lbs_df, n_classes_list):
    for i, level in enumerate(['subclass', 'order', 'superfamily', 'family', 'genus', 'species']):
        ...
        assert max_label < n_class and min_label >= 0, \
            f"错误：{level}标签范围[{min_label}, {max_label}]超出类别数{n_class}"
...
trained_coi_model = train_coi_only(
    ...
    full_data=full_data,
    config=final_config,
    best_model_path=f"{model_out_fold}/stage0_coi_only.pt"
)
...
multimodal_model = CurriculumDMGHAN(final_config, curriculum_stage=1).to(DEVICE)
